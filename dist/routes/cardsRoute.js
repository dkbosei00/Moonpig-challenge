...
router.get('/cards', async (req, res) => {
    ...
    try {
        ...
        const [cardData, templateData] = await axios.all(urls.map((url) => axios.get(url)));
...
router.get('/cards/:cardId/:sizeId?', async (req, res) => {
    ...
    try {
        ...
        const [cardData, templateData, sizesData] = await axios.all(urls.map((url) => axios.get(url)));
        // Check if data was successfully retrieved from the responses
        if (!cardData || !templateData || !sizesData) {
            return res.status(400).json({ message: "No data found." });
        }
        // Find the card by its ID in cardData
        const card = cardData.data.find(({ id }) => id === cardId);
        ...
        const response = {
            ...
            availableSizes,
            imageUrl,
            price: price ? `£${(price / 100).toFixed(2)}` : `£${(basePrice / 100).toFixed(2)}`,
            pages: pagesData
        };
